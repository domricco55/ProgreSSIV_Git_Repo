...
void T3SPI::begin_SLAVE() {//BEGIN SLAVE MODE
    setMCR(SLAVE);
...
void T3SPI::setCTAR(bool CTARn, uint8_t size, uint8_t dataMode, uint8_t bo, uint8_t cdiv){
    if (CTARn == 0){
        SPI0_CTAR0=0;}//THIS CLEARS THE ENTIRE SPI0_CTAR0 REGISTER
    if (CTARn == 1){
        SPI0_CTAR1=0;}
    setFrameSize(CTARn, (size - 1));//THIS SETS THE BITS FOR FRAME SIZE (The value in this register plus one is the frame size)
    setMode(CTARn, dataMode);//GET THIS CODE
