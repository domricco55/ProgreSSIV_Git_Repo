...
class CAN_filter_task
{
  ...
  private:
    ...
    volatile int16_t *node_rpms; //Pointer to an array of node rpms within an SPI_sensor_data_t type struct. This task will fill it with wheel velocities to be sent over spi.
  ...
  public:
    ...
    CAN_filter_task(node_info_t *node_info, volatile int16_t *node_rpms);
